...
# Install Docker
sudo for pkg in docker.io docker-doc docker-compose podman-docker containerd runc; sudo apt-get remove $pkg; 
sudo apt-get install ca-certificates curl gnupg -y
sudo install -m 0755 -d /etc/apt/keyrings
sudo curl https://download.docker.com/linux/debian/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg
...
# sudo docker run -d --name es01 -p 9200:9200 docker.elastic.co/elasticsearch/elasticsearch:8.10.3
